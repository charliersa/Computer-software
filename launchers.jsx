...
function PracticeLauncher({ctx, initial, onClose}){
  const { stats, startQuiz, DATA } = ctx;
  const [scope, setScope] = uS(initial?.type || 'all');
  const [item, setItem] = uS(initial?.item || '01');
  const [count, setCount] = uS(20);
  const [order, setOrder] = uS('random');
  ...
  const launch = ()=>{
    ...
    startQuiz({ mode:'practice', scope:{type:scope,item}, count: Math.min(count,poolSize),
      order, scopeLabel: lbl });
  };
  ...
  const chip = (active,disabled)=>({display:'inline-flex',alignItems:'center',gap:7,
  ...
  return React.createElement('div',{style:{padding:'30px 32px 28px'}},
    ...
    React.createElement(Row,{title:'測驗範圍'},
      React.createElement('div',{style:{display:'flex',gap:9,flexWrap:'wrap'}},
        scopes.map(s=>{ const dis = (s.k==='star'||s.k==='wrong') && s.n===0;
    ...
    scope==='item' && React.createElement(Row,{title:'選擇工作項目'},
      React.createElement('div',{style:{display:'grid',gridTemplateColumns:'1fr 1fr',gap:9}},
        DATA.items.map(it=>{ const m=ITEM_META[it.num]; const active=item===it.num;
    ...
    React.createElement(Row,{title:'題數'},
      React.createElement('div',{style:{display:'flex',gap:9,flexWrap:'wrap'}},
        [...counts,poolSize].map((c,i)=>{ const label = i===counts.length?('全部 '+poolSize):c;
...
/* ---------- Exam intro modal ---------- */
function ExamIntro({ctx, onClose}){
  const { startQuiz } = ctx;
  const rules = [
  ...
  return React.createElement('div',{style:{padding:'0 0 28px'}},
    ...
    React.createElement('div',{style:{display:'flex',gap:11,justifyContent:'flex-end',padding:'24px 32px 0'}},
      React.createElement('button',{className:'btn btn-quiet',onClick:onClose},'取消'),
      React.createElement('button',{className:'btn btn-dark btn-lg',onClick:()=>startQuiz({mode:'exam',
        scope:{type:'all'},count:80,order:'random',timeLimit:60*60,scopeLabel:'模擬考'})},
        IconWrap('ArrowR',{size:18}),'開始模擬考')));
}

Object.assign(window,{ PracticeLauncher, ExamIntro });
