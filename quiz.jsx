/* ============================================================
   QuizScreen — runner for practice & exam modes
   ============================================================ */
const { useState:uSq, useEffect:uEq, useRef:uRq, useMemo:uMq, useCallback:uCq } = React;

function QuizScreen({ctx}){
  const { store, patch, recordAnswer, toggleStar, setNote, finishSession, go } = ctx;
  const sess = store.active;
  if(!sess) { return React.createElement('div',{style:{padding:60,textAlign:'center'}},'沒有進行中的測驗。',
    React.createElement('div',{style:{marginTop:16}}, React.createElement('button',{className:'btn btn-primary',onClick:()=>go({name:'home'})},'回首頁'))); }

  const exam = sess.mode==='exam';
  const questions = uMq(()=> sess.qIds.map(id=>Q_BY_ID[id]), [sess.qIds]);
  const N = questions.length;

  const [idx,setIdx] = uSq(sess.idx||0);
  const [answers,setAnswers] = uSq(sess.answers||{});
  const [locked,setLocked] = uSq(sess.locked||{});
  const [now,setNow] = uSq(Date.now());
  const submittedRef = uRq(false);
  ...
  /* persist session */
  uEq(()=>{ patch(n=>{ if(n.active) n.active = {...n.active, idx, answers, locked}; }); },[idx,answers,locked]);

  /* timer tick */
  uEq(()=>{ const t=setInterval(()=>setNow(Date.now()),500); return ()=>clearInterval(t); },[]);
  const elapsed = (now - sess.startTs)/1000;
  const remaining = sess.timeLimit ? Math.max(0, sess.timeLimit - elapsed) : null;

  /* ----- compute & submit ----- */
  const doSubmit = uCq((auto)=>{
    ...
    questions.forEach(qq=>{
      ...
      const ok = p===qq.a;
      if(ok) correct++;
      if(exam && p) recordAnswer(qq, p);   // practice already recorded on lock
  ...
  uEq(()=>{ if(remaining!==null && remaining<=0 && !submittedRef.current) doSubmit(true); },[remaining]);
  ...
  const [confirmOpen,setConfirmOpen] = uSq(false);
  ...
  uEq(()=>{
    const h=(e)=>{
      ...
      if(['1','2','3','4'].includes(e.key)){ e.preventDefault(); choose(+e.key); }
        ...
        if(exam) next(); else if(isLocked) next(); }
    ...
    React.createElement('div',{style:{flex:1,maxWidth:1240,width:'100%',margin:'0 auto',padding:'28px 24px 90px',
      ...
      React.createElement('div',{key:q.id,className:'fade-in',style:{maxWidth: exam?'none':760, margin:exam?0:'0 auto', width:'100%'}},
        React.createElement(QuestionCard,{q,m,idx,exam,pick,isLocked,starred,onChoose:choose,
          onStar:()=>toggleStar(q.id),
          note: store.notes[q.id]||'', onNote:(t)=>setNote(q.id,t)}),
        /* nav footer */
...
function Timer({exam,remaining,elapsed}){
  ...
  return React.createElement('div',{className:'tag tnum',style:{fontSize:14,padding:'8px 13px',gap:7,
    background:'var(--ink-100)',color:'var(--ink-500)'}}, IconWrap('Clock',{size:15}), fmtTime(elapsed));
}
