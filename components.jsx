...
const Icons = {
  ...
  Target: ico([{tag:'circle',cx:12,cy:12,r:8},{tag:'circle',cx:12,cy:12,r:4},{tag:'circle',cx:12,cy:12,r:.6}]),
...
function IconWrap(name,props){ const C=Icons[name]; return C?React.createElement(C,props):null; }

/* ---------- Ring progress ---------- */
function Ring({value=0, size=64, stroke=7, color='var(--brand-600)', track='var(--ink-200)', children}){
...
/* ---------- Linear bar ---------- */
function Bar({value=0, color='var(--brand-600)', h=8}){
...
const fmtTime = s => { s=Math.max(0,Math.round(s)); const m=Math.floor(s/60); const ss=s%60;
  return (m<10?'0':'')+m+':'+(ss<10?'0':'')+ss; };
const OPT_LABELS=['①','②','③','④'];

Object.assign(window,{ Icons, IconWrap, Ring, Bar, ITEM_META, fmtTime, OPT_LABELS });
