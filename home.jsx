...
function HomeScreen({ctx}){
  ...
  const [modal, setModal] = uSh(null); // 'practice'|'exam'|{practice with item}
      ...
      React.createElement('section',{style:{display:'grid',gridTemplateColumns:'1fr auto',gap:24,
        ...
        React.createElement('div',null,
          ...
          React.createElement('div',{style:{display:'flex',gap:34,flexWrap:'wrap'}},
            [['累計作答',stats.answered,'tnum'],['正確率',pct(stats.accuracy)+'%'],
             ['待複習',stats.wrongIds.length],['已標記',stats.starredIds.length]].map((s,i)=>
              React.createElement('div',{key:i},
                React.createElement('div',{className:'tnum',style:{fontSize:27,fontWeight:800,color:i===1&&stats.answered?(stats.accuracy>=.6?'var(--ok-600)':'var(--bad-600)'):'var(--navy-800)'}}, s[1]),
...
function ItemProgress({ctx, onPick}){
  ...
  return React.createElement('div',{className:'card',style:{padding:'22px 24px'}},
    ...
    React.createElement('div',{style:{display:'flex',flexDirection:'column',gap:4}},
      DATA.items.map(it=>{ const m=ITEM_META[it.num]; const p=stats.perItem[it.num];
  ...
  return React.createElement('div',{className:'card',style:{padding:'22px 24px'}},
    ...
    sessions.length===0
      ...
      : React.createElement('div',{style:{display:'flex',flexDirection:'column',gap:8}},
          sessions.map((s,i)=>{ const pc=Math.round(s.correct/s.total*100); const pass=pc>=60;
            return React.createElement('div',{key:i,style:{display:'flex',alignItems:'center',gap:13,
              ...
              React.createElement('div',{style:{width:42,height:42,borderRadius:10,flex:'none',display:'grid',placeItems:'center',
                ...
                fontWeight:800,fontSize:14,className:'tnum'}}, React.createElement('span',{className:'tnum'},pc)),
              React.createElement('div',{style:{flex:1}},
