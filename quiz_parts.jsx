/* ============================================================
   Question card + answer options + feedback + exam palette
   ============================================================ */
function QuestionCard({q,m,idx,exam,pick,isLocked,starred,onChoose,onStar,note,onNote}){
  return React.createElement('div',{className:'card',style:{padding:'30px 34px 34px',boxShadow:'var(--sh-2)'}},
    ...
    React.createElement('div',{style:{display:'flex',flexDirection:'column',gap:11}},
      q.o.map((text,i)=>{ const n=i+1;
        let cls='default';
        if(isLocked){ if(n===q.a) cls='correct'; else if(n===pick) cls='wrong'; else cls='dim'; }
        else if(pick===n) cls='selected';
        return React.createElement(Option,{key:i,n,text,cls,exam,locked:isLocked,
          onClick:()=>onChoose(n)});})),
...
function Option({n,text,cls,onClick,locked}){
  const palette = {
...
function Feedback({q,m,pick,note,onNote}){
  const ok = pick===q.a;
  const [saved,setSaved] = React.useState(false);
  const tRef = React.useRef();
  const handle = (e)=>{ onNote(e.target.value); setSaved(true); clearTimeout(tRef.current);
  ...
  return React.createElement('div',{className:'fade-in',style:{marginTop:22}},
    ...
    React.createElement('div',{style:{marginTop:14}},
      ...
      React.createElement('textarea',{value:note,onChange:handle,rows:2,placeholder:'在此加註記憶點、觀念或為什麼選錯…（自動儲存於本機）',
...
/* ---------- exam palette ---------- */
function Palette({ctx,questions,answers,idx,setIdx,answered,onSubmit}){
  ...
  return React.createElement('aside',{style:{position:'sticky',top:86}},
    React.createElement('div',{className:'card',style:{padding:'18px 18px 20px'}},
      React.createElement('div',{style:{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:4}},
        ...
        React.createElement('span',{className:'tnum',style:{fontSize:13,color:'var(--ink-500)',fontWeight:700}},
          answered+' / '+N)),
      React.createElement('div',{style:{marginBottom:14}}, React.createElement(Bar,{value:answered/N,color:'var(--navy-800)',h:6})),
      React.createElement('div',{style:{display:'grid',gridTemplateColumns:'repeat(8,1fr)',gap:7}},
        questions.map((qq,i)=>{ const done=!!answers[qq.id]; const cur=i===idx; const star=!!ctx.store.starred[qq.id];
          return React.createElement('button',{key:qq.id,onClick:()=>setIdx(i),title:'第'+(i+1)+'題',
            style:{position:'relative',aspectRatio:'1',borderRadius:8,fontSize:12.5,fontWeight:700,
              className:'tnum',display:'grid',placeItems:'center',transition:'all .1s',
...
function Legend({c,label,bd,dot}){
...
Object.assign(window,{ QuestionCard, Option, Feedback, Palette, Legend });
