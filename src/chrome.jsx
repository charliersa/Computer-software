...
export function Brand({ small }) {
  return React.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: 12 } },
    ...
    React.createElement('div', { style: { lineHeight: 1.15 } },
      React.createElement('div', { style: { fontWeight: 800, fontSize: small ? 15 : 16.5, color: 'var(--navy-900)', letterSpacing: '.2px' } },
        '電腦軟體應用 ',
...
export function TopBar({ ctx, right, onBrand }) {
...
export function Modal({ open, onClose, children, width = 560, label }) {
  useEffect(() => {
    if (!open) return;
